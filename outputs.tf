output "endpoints" {
  ...
  vault_2 (${aws_instance.vault-server[0].public_ip}) | internal: (${aws_instance.vault-server[0].private_ip})
    ...
    $ ssh -l ubuntu ${aws_instance.vault-server[0].public_ip} -i ${var.aws_key_pair_key_name}.pem

    # Root token:
    $ ssh -l ubuntu ${aws_instance.vault-server[0].public_ip} -i ${var.aws_key_pair_key_name}.pem "cat ~/root_token"
    # Recovery key:
    $ ssh -l ubuntu ${aws_instance.vault-server[0].public_ip} -i ${var.aws_key_pair_key_name}.pem "cat ~/recovery_key"
  ...
  vault_3 (${aws_instance.vault-server[1].public_ip}) | internal: (${aws_instance.vault-server[1].private_ip})
    ...
    $ ssh -l ubuntu ${aws_instance.vault-server[1].public_ip} -i ${var.aws_key_pair_key_name}.pem

  vault_4 (${aws_instance.vault-server[2].public_ip}) | internal: (${aws_instance.vault-server[2].private_ip})
    - Started
    - You will join it to cluster started by vault_2

    $ ssh -l ubuntu ${aws_instance.vault-server[2].public_ip} -i ${var.aws_key_pair_key_name}.pem
